    ...
    <div class="banner-content">
        ...
        <div class="banner-abs">
            <div class="banner-abs-box">
                <div class="banner-abs-title">盈利宝金融网历史年化收益率</div>
                <b>{{ platInfo.avgRate }}<i>%</i></b>
                <p>平台用户数</p>
                <span>{{ platInfo.userCount }}<i>位</i></span>
    ...
    <div class="content">
        ...
        <div class="new-user" v-for="product in xinShouBao" :key="product.id">
            ...
            <div class="new-user-number">
                ...
                <a href="javascript:void(0)" class="new-user-btn" @click="goLink('/page/product/detail',{productId:product.id})">立即投资</a>
            </div>
...
export default {
    ...
    mounted(){
        //发送axios请求从服务器中获取首页的利率数据
        doGet("/v1/plat/info").then(res=>{
            // console.log('从服务器获得的数据'+ res.data.data)
            this.platInfo = res.data.data
        }),

        doGet("/v1/plat/index").then(res=>{
            // console.log('从服务器获得的数据'+ res)
            this.xinShouBao = res.data.data.xinshoubao
            this.youXuan = res.data.data.youxuan
            this.sanBiao = res.data.data.sanbiao
        })
